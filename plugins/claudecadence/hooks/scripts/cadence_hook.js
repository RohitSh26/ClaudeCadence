...
function bootstrapProject() {
  ...
  // v2.7.1: drop a self-ignoring .gitignore inside the cadence dir. The
  // sole rule `*` makes git treat everything in here — viewer files,
  // nodes.js, image attachments, the .gitignore itself — as ignored,
  // regardless of what the project's root .gitignore looks like. Users
  // can never accidentally `git add` 4 MB of session log + screenshots.
  const cadenceGitignore = path.join(CADENCE_DIR, '.gitignore');
  if (!fs.existsSync(cadenceGitignore)) {
    try {
      fs.writeFileSync(cadenceGitignore,
        '# Auto-generated by ClaudeCadence — keeps this directory out of git.\n' +
        '# Everything in here is local session state (nodes.js, images, viewer\n' +
        '# files, bookkeeping). Do not commit. Safe to delete; bootstrap rewrites\n' +
        '# this file on the next hook fire.\n' +
        '*\n'
      );
    } catch (_) { /* fail-soft */ }
  }
